...
(function() {
  ...
  var $about = document.getElementById('aboutDrop');
  ...
  (function init() {
    ...
    if ($toc) {
      ...
      var topHeight = parseInt($wrapper.offsetTop) - 60;
      if(winWidth > 930){
        $body.scrollTop > topHeight ? Util.addClass($toc, 'fixed') : Util.removeClass($toc, 'fixed');
        $body.scrollTop > topHeight ? Util.addClass($scrollHeader, 'scroll-header') : Util.removeClass($scrollHeader, 'scroll-header');
      }
    }

    var url = window.location.href;
    var urlArray = url.split('//');
    var para = urlArray[1].split('/');
    if(para.indexOf('archives') > -1 || para.indexOf('tags') > -1){
      $('#menu').find('.nav').eq(1).addClass('active');
    }else{
      $('#menu').find('.nav').eq(0).addClass('active');
    }
  }());

  if($aboutNav) {
    Util.bind($aboutNav, 'click', function() {
      Util.removeClass($about, 'hide');
    });

    Util.bind($closeBtn, 'click', function() {
      Util.addClass($about, 'hide');
    });
  }
  
  if($menu) {
    Util.bind($menuToggle, 'click', function () {
      Util.toggleClass($menu, 'toggle');
    });
  }
  ...
  Util.bind(window, 'scroll', function() {
    ...
    if ($toc) {
      ...
      var topHeight = parseInt($wrapper.offsetTop) - 60;
      if(winWidth > 930){
        scrollTop > topHeight ? Util.addClass($toc, 'fixed') : Util.removeClass($toc, 'fixed');
        $body.scrollTop > topHeight ? Util.addClass($scrollHeader, 'scroll-header') : Util.removeClass($scrollHeader, 'scroll-header');
